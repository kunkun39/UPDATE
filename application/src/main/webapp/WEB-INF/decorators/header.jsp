...
<div class="header">
    ...
    <div class="menu">
        ...
        <security:authorize ifAnyGranted="ROLE_SYSTEM">
            <a href="${pageContext.request.contextPath}/backend/productmanagement.html" <c:if test="${'PRODUCT' eq BROSWER_LOCATION}">style="font-weight:bold;background:#5d5c5c;"</c:if>>产品管理</a>
        </security:authorize>

        <security:authorize ifAnyGranted="ROLE_DEVICE">
            <a href="${pageContext.request.contextPath}/backend/myresponsibleproduct.html" <c:if test="${'MY' eq BROSWER_LOCATION}">style="font-weight:bold;background:#5d5c5c;"</c:if>>升级管理</a>
        </security:authorize>

        <a href="${pageContext.request.contextPath}/backend/systemclientupdatereport.html" <c:if test="${'STA' eq BROSWER_LOCATION}">style="font-weight:bold;background:#5d5c5c;"</c:if>>统计管理</a>
